        ...
        <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
            <div class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                @foreach($categories as $category)
                    <a wire:navigate class="group flex flex-col bg-white border shadow-sm rounded-xl hover:shadow-md transition dark:bg-slate-900 dark:border-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="{{ url('/' . currentCompany()->slug . '/products') }}?selected_categories[0]={{ $category->id }}" wire:key="{{ $category->id }}">
